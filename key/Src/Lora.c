...
//目标设备的lora addr
static uint16_t u16TargetID = 0;
static bool bPressFlag = false;
...
static bool parsingdata(char *buff)
{
  char *str;
  
  if ((str = strstr(buff, "WM+"))) {
    if((str = strstr(str, "OD+"))) {
      return true;
...
void LoraModuleTask(void)
{
  ...
  uint8_t buff[40] = {0};   //保存从数据池中取得的数据  
  //接收数据的处理
  ...
  if (DataPoolGetNumByte(dp, &buff[0], 38)) {
    ...
    char bull[80] = {0};     //接收到的数据是去零数据，需要在对应位置填充零
    char plaintext[80] = {0};//解密后得到的字符串数据
    uint8_t i,j = 0;
...
void setLockID(uint16_t id)
{
  u16TargetID = id;
}

void GetButtonPressHandler(void)
{
  bPressFlag = true;
}

//循环发送开门和关门信息
void LoraSendMsg(uint16_t id)
{
  ...
  RSA_Encrypt(str, strlen(str), (int *)buffer);
  
  sendmsg[0] = (uint8_t)(id >> 8);
  sendmsg[1] = (uint8_t)id;
  sendmsg[2] = 0x1c;
  //提取出非零元素
  for(i=0,j=0; i < 38; i+=2,j++) {
    sendmsg[3 + i + 0] = buffer[j*4 + 0];
    sendmsg[3 + i + 1] = buffer[j*4 + 1];
  }

  set_loramodule_workmode(lm, LoraMode_WakeUp);
  HAL_Delay(5);
  HAL_UART_Transmit(um->uart, sendmsg, sizeof(sendmsg), 100);
  HAL_Delay(5);
  while(!get_loramodule_idle_flag());
  set_loramodule_workmode(lm, LoraMode_LowPower);
}
...
void afterWakeUp(void)
{
  ...
  if ( bPressFlag ) {
    ...
    //判断是控制唤醒还是设置唤醒
    //控制唤醒：就是直接控制锁的开与关
    //设置唤醒：就是对拷，需要将另一个key发送的信息保存本key上
    LoraSendMsg(u16TargetID);
//    printf("lora send msg\r\n");
    bPressFlag = false;
  } else { 
    LoraModuleTask();
  }
